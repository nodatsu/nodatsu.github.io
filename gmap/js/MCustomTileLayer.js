...
function addOverlay(theme, title, value){
...
MCustomTileLayer.prototype.getTileUrl = function(p,z) {
	var rnd = ''; if (debugmap) { rnd = '?r=' + Math.random(); }
	var url = './' + imgfoldr + '/' + this.theme + '/' + z + '/' + p.x + '_' + p.y +  '_' + z + '.png' + rnd;
	return url;
};
...
MCustomTileLayer.prototype.fadeToOpacity = function(op) {
	...
	if (this.opacity != this.targetOpacity) {
		...
		timelabel = window.setTimeout(function(){self.fadeToOpacity()}, 5);
	}
